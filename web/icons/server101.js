...
function normalizeVehicleType(v) {
  ...
  if (s.includes('pickup')) return 'Honda';
...
//////////////  calle helper from sms regarding the cancelation of rider /////////////
import { cancelRideBeforePickup } from './sms.js';

// inside your route handling rider cancellation
app.post('/ride/cancel', async (req, res) => {
    const { rideId, riderPhone, driverPhone, mapUrl } = req.body;

    const cancelRes = await cancelRideBeforePickup({ rideId, riderPhone, driverPhone, mapUrl });

    if (!cancelRes.ok) {
        return res.status(500).json({ ok: false, error: cancelRes.error });
    }

    return res.json({ ok: true, message: 'Driver and rider notified' });
});
////////////////////////////////////////////
...
const requestedVehicle = normalizeVehicleType(typeVehicleRaw);

console.log('[DISTANCE BODY KEYS]', Object.keys(req.body || {}));
console.log('[FARE INPUT]', {
  pickup,
  destination,
  typeVehicle_raw: typeVehicleRaw,
  typeVehicle_norm: requestedVehicle,
});
...
const mult = VEHICLE_MULT[requestedVehicle] || 1.0;

const distanceCost = (PER_KM * mult) * km;
const timeCost = (PER_MIN * mult) * minutes;
const base = (BASE_FARE * mult) + distanceCost + timeCost;

let fare = base * TAX_MULTIPLIER;

if (km >= 14) fare = fare * 0.90;
...
app.post('/notify-driver', async (req, res) => {
  try {
    ...
    const typeVehicleRaw =
      req.body?.type_vehicle ?? req.body?.vehicle_type ?? req.body?.selectedVehicleType ?? null;
...
try {
  const msg = await sendSms({
    ...
    body:
      ...
      `Répondez OUI / YES / نعم pour accepter.\n` +
      `Job ID: ${rideIdNum}`
  });
...
export async function sendOffer(jobId) {
  ...
  if (!job) {
    ...
    job = {
      ...
      triedDrivers: Array.isArray(ride.tried_drivers) ? ride.tried_drivers : [],
      typeVehicle: ride.vehicle_type || null
    };
...
app.post('/driver/heartbeat', async (req, res) => {
  ...
  try {
    ...
    // Normalize phone (strip whatsapp: if present)
    const phoneNorm = String(phoneRaw || '').startsWith('whatsapp:')
      ? String(phoneRaw).slice('whatsapp:'.length).trim()
